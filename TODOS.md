...
**Status:** In progress — PR #14 (`feat/dengonban-board`, open, CI green) has the schema, write/moderation RPCs, client wiring, and the public `board.html` UI. Not done yet: the `admin.html` moderation panel (the `admin_list_dengonban`/`admin_hide_dengonban_message` RPCs exist and are wired client-side, just no UI consumes them), e2e test coverage, and applying the migration to the live Supabase project (no Docker available locally to verify via `supabase db push`/`db lint` — needs a real apply + smoke test before this is functionally live even after merge).
...
**Context:** Reference images gathered 2026-08-19 (Google Images search for "rengonban 伝言板") — real dengonban boards are a green chalkboard/whiteboard ruled into a grid (date/time/message columns), handwritten, physically erased over time. Built as: `supabase/migrations/20260824000000_dengonban.sql` (table + RLS + rate-limited `post_dengonban_message` RPC, reusing the existing `check_rate_limit()` from T14 since posting requires auth — no new infra needed), messages auto-expire after 30 days (the physical-erasure trait carried into the mechanic, not just flavor text), and `board.html` (public read, member-only compose, inverted chalkboard-style panel, linked from the site nav and every page's footer).
**Effort:** L
**Priority:** P3
**Depends on:** None
